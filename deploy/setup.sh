echo '* libraries/restart-without-asking boolean true' | sudo debconf-set-selections

#!/usr/bin/env bash

set -e

PROJECT_GIT_URL='https://github.com/billzyc/studyplan-rest-api.git'

PROJECT_SECRET_URL='https://github.com/billzyc/course_planner_secrets.git'

PROJECT_BASE_PATH='/usr/local/apps/course-planner-rest-api'

echo "Installing dependencies..."
apt-get update
apt-get install -y python3-dev python3-venv sqlite python-pip python3-pip supervisor nginx git

# Create project directory
mkdir -p $PROJECT_BASE_PATH
git clone $PROJECT_GIT_URL $PROJECT_BASE_PATH

# Create virtual environment
mkdir -p $PROJECT_BASE_PATH/env
python3 -m venv $PROJECT_BASE_PATH/env

# Install python packages
$PROJECT_BASE_PATH/env/bin/pip3 install -r $PROJECT_BASE_PATH/requirements.txt
$PROJECT_BASE_PATH/env/bin/pip3 install uwsgi==2.0.18

# Clone secrets folder

cd $PROJECT_BASE_PATH/course_planner
git clone $PROJECT_SECRET_URL

# Run migrations and collectstatic
cd $PROJECT_BASE_PATH
$PROJECT_BASE_PATH/env/bin/python3 manage.py migrate
$PROJECT_BASE_PATH/env/bin/python3 manage.py collectstatic --noinput

# Configure supervisor
cp $PROJECT_BASE_PATH/deploy/supervisor_course_planner_api.conf /etc/supervisor/conf.d/course_planner_api.conf
supervisorctl reread
supervisorctl update
supervisorctl restart course_planner_api

# Configure nginx
cp $PROJECT_BASE_PATH/deploy/nginx_course_planner_api.conf /etc/nginx/sites-available/course_planner_api.conf
rm /etc/nginx/sites-enabled/default
ln -s /etc/nginx/sites-available/course_planner_api.conf /etc/nginx/sites-enabled/course_planner_api.conf
systemctl restart nginx.service

echo "Setup complete"